"""
청라 식물공장 통합 모니터링 v3
실행: python app_rt_v3.py
접속: http://127.0.0.1:8050

변경사항 (v3):
  - 헤더 타이틀 / 부제 크기 확대 + 가운데 정렬
  - 탭 레이블 크기 확대 + 가운데 정렬
  - 재배 현황 색상 범례 재설계
      0~7일   (정식 초기)
      8~14일  (활착기)
      15~21일 (팁번 발생 초기) ← 연두-노랑 중간색 + 경고 의미
      22~28일 (팁번 경고)
      29일+   (수확 권장)
  - 온습도 도면 초기값 직접 주입 → 탭 클릭 시 즉시 표시 (app_v2 핵심 수정 유지)
"""
...
# ─────────────────────────────────────────────
# 3. 색상 단계 정의 (재배 현황 도면 & 범례 공용)
# ─────────────────────────────────────────────
# (최대일수, 배경색, 텍스트색, 범례 레이블)
GROWTH_STAGES = [
    (  7, "#e3f2fd", "#1565c0", "0~7일   (정식 초기)"),
    ( 14, "#f1f8e9", "#33691e", "8~14일"),
    ( 21, "#dff2a8", "#5a7a00", "15~21일 (팁번 발생 초기)"),   # 연두-노랑 중간
    ( 28, "#fff0a0", "#b07800", "22~28일 (팁번 경고)"),
    (999, "#ffd180", "#8c3a00", "29일+   (수확 권장)"),
]

def days_to_stage_colors(plant_days):
    """정식 후 경과일 → (fill색, 텍스트색)"""
...
def make_stats_card(values, mode):
    unit = "°C" if mode == "temp" else "%"
    if not values:
...
def make_time_series(bed_id, mode, season):
    ...
    fig.update_layout(
        ...
        hovermode="x unified", legend=dict(font=dict(size=11)),
    )
    return fig


# ─────────────────────────────────────────────
# 8. 재배 현황 도면 Figure 생성
# ─────────────────────────────────────────────
...
def make_cultivation_figure():
    ...
    for bed_id, (cx, cy, w, h) in BED_LAYOUT.items():
        ...
        fill = GROWTH_STAGES[-1][1]  # 기본값: 수확 권장색
...
def make_legend():
    """GROWTH_STAGES에서 자동으로 범례 생성 → 색상 정의와 항상 동기화"""
    return html.Div(
        [html.Span("■ 색상 범례:  ",
                   style={"fontWeight": "700", "fontSize": "12px", "color": "#4a5568",
                          "marginRight": "6px"})]
        + [html.Span(
            f"■ {label}",
            style={
...
app.layout = html.Div([
    ...
    # ── 탭 ────────────────────────────────────
    dcc.Tabs(
        id="main-tabs", value="tab-temp",
        children=[
            dcc.Tab(label="🌡 온도 분포", value="tab-temp",
                    style=TAB_STYLE, selected_style=TAB_SEL_STYLE),
            dcc.Tab(label="💧 습도 분포", value="tab-hum",
                    style=TAB_STYLE, selected_style=TAB_SEL_STYLE),
        ...
        style={
            ...
            "justifyContent": "center",     # ★ 탭 전체 가운데 정렬
        },
    ),
...
def render_tab(tab):
    """
    ★ 핵심: suppress_callback_exceptions=True 환경에서
       dcc.Graph의 figure= 에 초기값을 직접 주입해야
       탭 클릭 시 그래프가 즉시 표시된다.
    """
    if tab in ("tab-temp", "tab-hum"):
        mode = "temp" if tab == "tab-temp" else "hum"

        # 초기 도면 & 통계 카드 직접 계산 후 주입
        init_values = get_values(mode, INIT_HOUR, INIT_SEASON)
        init_fig    = make_floor_figure(init_values, mode, INIT_HOUR, "전체 평균")
        init_card   = make_stats_card(init_values, mode)
        ...
        return html.Div([
            ...
            html.Div([
                html.Div([
                    html.Label("🍂 계절",
                               style={"fontWeight": "600", "fontSize": "12px",
                                      "color": "#4a5568"}),
                    dcc.Dropdown(
                        id="season-dd", options=SEASON_OPTIONS,
                ...
                html.Div([
                    ...
                    dcc.Slider(
                        id="hour-slider", min=0, max=23, step=1, value=INIT_HOUR,
                ...
                html.Div([
                    html.Button(
                        "▶ 재생", id="play-btn", n_clicks=0,
                        style={"background": "#3182CE", "color": "white",
                               "border": "none", "borderRadius": "6px",
                               "padding": "8px 16px", "cursor": "pointer",
                               "fontSize": "13px", "fontWeight": "600"},
                    ),
                    dcc.Interval(id="anim-interval", interval=800,
            ...
            # 메인 영역
            html.Div([
                # ★ figure= 초기값 직접 주입 → 탭 전환 시 즉시 표시
                dcc.Graph(id="floor-graph", figure=init_fig,
...
def upd_floor(mode, hour, season):
    ...
    values = get_values(mode, h, season)
    return make_floor_figure(values, mode, h, sl), make_stats_card(values, mode)
